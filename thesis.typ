...
Such limitation deprives the software and its users, including online educational platforms and learning management software such as Artemis #footnote("https://github.com/ls1intum/Artemis") which integrate Apollon, from the benefits of a collaborative learning environment, wherein students work in groups towards solving a problem or accomplishing a shared task. These collaborative learning techniques have been shown to increase academic motivation @collab-learning and peer contribution @collab-on-table, effects that specifically in a blended learning environment, become more pronouned based on how well the tools in use are optimised for collaboration @collab-learning-blended.

Implementation of such features, on the other hand, brings its own unique challenges, specifically for the context of Apollon. A key challenge in such a context is maintaining a consistent state between all peers: while working on the same UML diagram, all participants should see the same diagram, while being able to quickly and responsively modify the diagram without having to lock the diagram or wait for other peers, meaning occasionally changing the diagram simultaenously, resulting in conflicting states. Common solutions for such problems, such as Operation Transformation @ot, demand a lot of  complexity for conflict detection and reconcilliation, which have proven difficult and error-prone to even merely check for correctness @ot-issues, @ot-admis, @ot-proof, and demand strong network guarantees and low fault tolerance @diff-sync, @ot. Tools built around these approaches typically require tight server-client integration @sharedb, increase the difficulty of integration and use for a tool such as Apollon, where third-party consumers should integrate the tool and handle server-side operations.

More recent solutions such as Conflict-free Replicated Data Types (CRDTs) @og-crdt, @crdts are more promising for the specific requirements of a project such as Apollon. The "conflict-free" nature of such solutions greatly simplifies complexity for consumers and integrators. However, they still incur noticeable trade-offs in terms of performance and complexity @yata, posing further challenges.
...
1. *Design and Implementation* \
  The main aim of the thesis is to design and implement a realtime collaboration solution for Apollon. The solution should impose minimal complexities on potential consumers of Apollon intending to integrate their tool into their own systems, and subsequently will draw inspiration from more distributed solutions such as CRDTs. At the same time, we intend to minimise the complexity incurred on Apollon's code base itself, providing a solution that works for the wide variety of diagram types supported by Apollon out of the box, working seamlessly for future diagram types without additional effort, and retaining a simple architecture maintainable considering the student-driven high-influx development process of Apollon.

#v(4mm)

2. *Integration* \
  The second objective would be to nominally integrate the realtime collaboration features of Apollon into example consumers of the package, namely Apollon Standalone, a standalone version of Apollon, and Artemis, an interactive learning management system. The unqiue constraints of each of these systems will act as a testbed for simplicity of adoption of the proposed solution, and helps in analysing potential challenges related to integration of realtime collaboration in different environments and for various use cases and in accordance with different persistence mechanisms.

#v(4mm)

3. *Theoretical Analysis* \
  The final objective is to provide a theoretical analysis of the proposed solution, specifically focusing on the conditions under which the solution is applicable, and the boundaries within which the solution is correct. This analysis will describe consistency properties achievable using the proposed solution under various network topologies and constraints, and discuss the theoretical correlations of the proposed solution with established ones.
...
@background firstly provides some background information on Apollon editor and Artemis teaching platform. Concept of realtime collaboration is subsequently introduced, alongside consistency requirements often considered for such systems, and an overview on the most commonly used methodologies for satisfying these requirements and addressing associated challenges is given, namely Operational Transformation and Conflict-free Replicated Data Types.

@related-work provides a more in-depth review of these techniques and their suitability for the specific problems focused in this thesis, providing insight into similarities and differences of the proposed solution with existing approaches, and the reasoning as to why a novel solution was designed and implemented.

In @requirements-analysis, functional and non-functional solution requirements are outlined, as well as system models, following requirement ellicitation and analysis process outlined in @bruegge2004object.

@system-design describes the system design, including the architectural design of the proposed solution, the data format of diagrams and changes and their interactions with the realtime collaboration solution, the control flow of the subsystem, and an overview of example integrations of the proposed system into Apollon Standalone and Artemis projects respectively.
...
- _realtime_: response to local user interaction should be immediate, i.e. a user shouldn't wait for approval from a central server to see the effect of their actions.
...
- _unconstrained_: multiple users should be allowed to modify various and potentially overlapping parts of the data concurrently.
...
Operational Transformation assumes a network reliable enough for delivering messages exactly once @ot, and are fault tolerant towards out-of-order delivery by transforming incoming operations in a manner that ensures commutativity, while the operations themselves more often lack that property @ot-proof. For example, in the scenario depicted in @figure-ot, while $O_1$ and $O_2$ aren't commutative, we do have $O_1 compose O'_2 equiv O_2 compose O'_1$. Ensuring this property with higher number of peers and increasing number of concurrent operations is specially challenging @ot-issues, and although distributed variants exist, many implementations (such as Google Docs) resort to sequencing and transforming operations centrally, on the server-side, to avoid these issues @ot-x-crdt.

== Conflict-free Replicated Data Types <crdt-background>

Conflict-free Replicated Data Types are a more recent family of solutions for achieving eventual consistency, based on (re)desigining the data type in a manner that operations are natively commutative @ot-x-crdt, @ot-v-crdt. This property ensures that operations can be exchanged in a distributed (peer-to-peer) manner, and all peers will converge to the same state.

The most basic example of such a data type is a counter @crdt-pure-op, where peers exchange "increment" and "decrement" operations. In a network with exactly-once-delivery, all peers eventually converge on the same value. Another simple example would be a 2P-Set @crdt-list, where elements can be added and removed, but not added back again. Each peer holds a main and a _tombstone_ set, upon receiving a "remove" op from another peer will add the element to its tombstone set and remove it from the main set, and upon receiving an "add" op will ad the element to the main set if it's not in its tombstone set #footnote([
  The limitation of adding back removed elements can also be circumvented using unique IDs, and storing IDs in the tomstone set.
]).
...
State-based CRDTs have a higher network fault tolerance as they do not require exactly-once-delivery guarantee. This however comes at the cost of higher message size and implementation complexity. It has been shown that CvRDTs and CmRDTs can emulate each other and are thus equivalent @crdts. The problem of message size of CvRDTs can be mitigated by sending _delta-states_ instead of full states, resulting in a third class called $delta$-CRDTs, though this technique also incurs its own complexities for handling update histories between peers @delta-crdt.

CRDTs can also be used to construct more complicated data types, even those whose operations are not naturally commutative (such as strings, arrays, sequences, etc) @crdt-list, @treedoc, @logoot, @yata. @figure-crdt-seq shows schematics of a CRDT sequence, wherein each element is assigned a unique ID from a set with some total ordering. When a character is inserted, its assigned ID is between those of its siblings at time of insertion: for example when inserting "a" between "x", "y", its ID is picked so that $"ID"_x < "ID"_a < "ID"_y$, ensuring preservation of intention. Deleted characters are merely marked as such, which similar to tomstoning of 2P-Set elements, handles out-of-order delivery of operations and conserving causality. 
...
Furthermore, while distributed designs for OTs have also been proposed @ot, checking correctness of such algorithms and covering all potential corner cases is quite complex and often error-pronse @ot-issues, @ot-proof, @ot-admis. The famous example of the dOPT puzzle is outlined in @figure-dopt-puzzle, where the originally proposed dOPT algorithm fails to achieve convergence.
...
CRDT-based solutions are another viable alternative for bringing realtime collaboration to Apollon. General-purpose and extensible solutions such as YATA @yata have been developed, and the distributed nature of CRDTs removes any burden of integration of realtime collaboration of Apollon from integrating platforms. That said, CRDTs introduce their own host of performance overheads and complexities @ot-v-crdt, @delta-crdt, @yata. Garbage collecting tombstones is a recurring problem in CRDTs, and while CvRDTs are more fault tolerant than CmRDTs or OTs, they incur a much higher message size and bandwidth consumption.
...
The purpose of this thesis is to design and implement support for realtime collaboration into Apollon, so that implementation of realtime collaboration for consumers integrating Apollon into their projects becomes a straightforward task. This means Apollon, in some form or manner, informing consumer code of "changes" happening to diagrams that need to be synced with potential remote peers, and providing a mechanism for importing such changes produced by a remote Apollon client. The implementation should yield at minimum Eventual Consistency, ideally Strong Eventual Consistency and Intention Preservation, with minimal complexity and overhead for consumer code, and minimal network guarantees. It should be designed in a manner that also allows third-party code to participate in any such network, primarily with purpose of remote persistence. We strive for an architecture that also incurs minimal complexity for Apollon itself, maintaining its extensible architecture and ease of adding new diagram types, without requiring developers to design such future capabilities with realtime collaboration and its associated challenges in mind. Such a design, ideally should be domain-agnostic enough to also be applicable to similar projects, even beyond the scope of UML diagrams.

== Current System <current-system>

This section describes the status of realtime collaboration in Apollon prior to the implementation work via this thesis. The current design lacks any direct support for realtime collaboration within Apollon, requiring integrating projects to implement and handle such a system from scratch. As described in @background, many such solutions (such as CRDTs) require specifically designed internal data types, which limits the range of possible solutions.

@figure-stquo outlines an example implementation of such a system without delving into specifics of the implementation. Apollon system consists of components necessary for rendering diagrams and providing interactions upon them. The UML model itself is managed via the _Core Model_ component, which follows a Flux-based data flow for managing the state @flux, and separates the internal representation of the UML model, which has additional rendering and interaction metadata, from the external representation exposed via _ApollonEditor_ interface.

#figure(
  image("figures/flux.svg", width: 80%),
  caption: [
...
The proposed system should shift responsibility of detecting changes, updating and preparing _update messages_, detecting concurrency and conflicts, and reconcilliating them into consistent diagrams across multiple remote peers, to a new component within Apollon itself (denoted as "Patcher" in @figure-proposed). Responsibilities such as displaying and detecting online peers and their status still remains with the integrating code. The "Patcher" component detects changes in the core model and notifies subscribers with _update_ or _sync_ messages (depending on the actual design), and handles incoming _update_ (or _sync_) messages from remote peers, properly updating the core model accordingly. These functionalities are provided as an independent component to the rest of Apollon subsystems, to maintain simplicity and extensibility.
...
The following subsections detail the requirements of the proposed system. First, the functional requirements are documented, describing interactions and functional behaviour of the realtime collaboration system with users and integrating code. Qualitiative requirements of the system are then detailed under non-functional requirements and categorized using the FURPS+ model as advised by @bruegge2004object.

#pagebreak()

=== Functional Requirements <functional-requirements>

The following requirements describe how the realtime collaboration component of Apollon should functionally behave towards users and integrating code (also refered to as "Consumer Code").
...
- FR2 *Realtime Updates*: Users should see the effect of changes they make immediately, regardless of network conditions. <fr2>
...
This section, as specified by @bruegge2004object, describes the aspects and requirements of the system that are not directly related to the functional behaviour of the system, but rather are indicative of its quality, usability, performance, etc. The requirements are categorized using the FURPS+ model.
...
- NFR3 *Reliability*: The system should be fault-tolerant towards network unreliabilities, including out-of-order delivery of messages, non-deterministic network delays, message loss, duplicate delivery, etc. Only eventual delivery of messages should be assumed, i.e. if a message is sent infinite times, it will eventually be delivered infinite times. <nfr3>
- NFR4 *Reliability*: Diagrams that users see and work on should be the same as much as possible. The ideal limit of this requirement would be for _Strong Eventual Consistency_ to be satisfied by the system, i.e. users that have received the same (unordered) set of updates (messages) see the same diagram. <nfr4>
...
*Demo Scenario*: This scenario describes the interaction between developers of Artemis, an online learning platform integrating Apollon, a developer of Apollon, adding a new diagram type to Apollon, and some students, working on a team exercise on Artemis platform, on this new diagram type.
...
*Students*: A group of students are working on a team BPMN modeling exercise on Artemis platform. Each student can make changes to any part of the diagram as they see fit, seeing the changes they make immediately on the diagram. They can also see the changes made by their peers, like when a new element is created, or a new relationship is made. Alice and Bob, simultaenously move two connected elements, and they see the elements moved as their intended with the other connected element also move as their peer intended.
...
These actors use functionality provided by two different scopes: _Apollon_, as the diagram editor which also provides features for realtime collaboration, and the _Integrating Platform_, which embeds Apollon into their own system and provide additional context-specific functionality.
...
The process starts with an action dispatched from the _View_ (step 1), potentially due to user interaction. This action is intercepted by the _Middleware_ layer, wherein it is processed by composed middlewares, including the _patcher middleware_ described in the previous section, which asks the _Patcher_ component to check for potential _patches_ that would need to be emitted (step 2). The action (or the processed action) is meanwhile passed to the _Reducer_ layer and its effect are applied to the _Store_ (step 1.a), resulting in the user receiving immediate realtime feedback of the changes they have issued.

The _patcher_ checks the action and, if necessary, emits _patches_, which are relayed through _ApollonEditor_ interface layer to the _Integrating Platform_ (steps 3 and 4). For simplicity, we've assumed a standard client-server architecture with a remote persistence layer for the integrating platform in this diagram. The _Integrating Platform_ then broadcasts the _patch_ to all remote peers, potentially including the originating client (depending on the designed solution and network requirements), and passes the _patch_ down to _ApollonEditor_ (step 7). An action is dispatched (step 8), which is intercepted by the _patcher reducer_, which utilising the _Patcher_ calculates necessary updates to the store (steps 9 and 10), which are then applied to the _Store_ and subsequently updated in the user interface. The process for handling incoming changes from a remote peer would be basically captured in steps 6 through 12.
...
- *Performance* \
  Performance directly ties to the previous design goals, as a system with a lot of overhead will have detimental effects on user experience and portability. For example, considering the web-based nature of Apollon and the wide-variety of devices that users might access it, reconcilliation strategies that are taxing on the client-side resources such as CPU or memory should be avoided, specifically strategies that scale resource consumption based on content-size or number of users (#link(<nfr6>)[NFR6]). Or, depending on network conditions, if a user sees the changes made by their peers too late, the collaborative nature of the setting is disturbed (#link(<nfr5>)[NFR5]). \
...
- *Portability* \
  Apollon is designed to be easily integrated into other platforms. Its realtime collaboration system should also be quite straightforward to implement, with minimal requirements on the integrating platform beyond the necessary message delivery mechanism (#link(<nfr8>)[NFR8]). It should not require any tight-coupling with the internal workings of the realtime collaboration solution, and should expose the system in a standard and interoperable manner, as integrating platforms might utilise the system for various use cases such as remote persistence (#link(<nfr9>)[NFR9]).

#v(2mm)

- *Extensibility* \
  Apollon is specifically designed to be extensible, specifically with regards to new diagram types, so any realtime collaboration solution must consider that and avoid tight-coupling with various features of Apollon, specifically various diagram types (#link(<nfr10>)[NFR10], #link(<nfr11>)[NFR11]).
...
The design goals described in the above section are outlined in order of priority for this work. Usability requirements are strong enough for a realtime collaboration system that many of them actually fall under functional requirements, as otherwise the definition of a realtime collaboration system would be violated. Performance requirements take second priority as they affect both usability and can have serious negative impact on portability, which considering the design and intended use of Apollon, taking the next priority. Extensibility requirements are the next priority, considering the extensible design of Apollon and its student-driven development model.

Reliability in face of various network conditions takes last priority. Though important, integrating platforms can be expected to setup normalised network systems such as a client-server network, which greatly simplifies many issues with distributed reconcilliation. Normal network conditions can also be expected in many cases: while out-of-order delivery will occur necessarily due to realtime nature of the system (a user sees their own changes always out-of-order with changes of other peers), properties such as exactly-once-delivery or a preserved deliver-order between the server and the clients can be expected (and are in fact the assumptions for popular solutions such as Operational Transformation @ot and Google Docs @ot-x-crdt). Subsequently, in trade-offs between performance, portability and additional reliability, the latter takes lower priority. That said, the design of the system should enable integrating platforms to mitigate such corner cases without requiring a tight coupling to the internals of the system.
...
The integrating platform interfaces with the realtime collaboration by subscribing to the _Patcher_ (for emitting local changes) or by dispatching _patch actions_ (for receiving remote changes), both through the _ApollonEditor_ interface. The integrating platform can then utilise this patch to update the local or remote storage, or broadcast the diagram to other peers. The integrating platform is required to deliver emitted patches to all clients, including the client issuing the patch (see @diagram-schema).
...
To mitigate this stuttering issue, the _Patch Verifier_ component was added to the realtime collaboration subsystem. When patches are to be emitted, the _Patcher_ asks the _Patch Verifier_ to sign each operation within the patch. The verifier checks each operation, and assigns a unique, randomly generated identifier to each replace operation (as other operations do not yield stuttering). The verifier also records the corresponding path of the patch with the unique identifier, for further verification of incoming patches (@figure-st-sign).
...
Mitigation for such conditions is not baked in to the proposed solution itself in favor of meeting the expressed design goals (specifically of performance, portability and extensibility). Instead, the system is designed in a manner that accomodate such mitigation strategies.

One naive strategy would be to repeat the latest operation of each path, in an unsigned manner, to each client until the client confirms receipt. While operations of differing paths might be delivered out-of-order, their commutativity ensures that the clients converge on the same state. The removal of signature also ensures the stuttering suppression mechanism doesn't overrule the patch. To respect stuttering prevention, the network layer can for example send the unsigned version of the patch with a set delay, avoiding the stuttering effect as well. This naive strategy ensures Eventual Consistency amongst clients in network conditions where merely eventual delivery is guaranteed.

The flexibility of the design of the system also allows integrating platforms to implement more sophisticated strategies. For example, using totally ordered logical timestamps allow the integrating platform to operate in a fully distributed manner, though incurring the associated overhead. The integrating platform can also detect prolonged disconnections of a client and resync the diagram wholesale. Further analysis and details of how various consistency properties can be achieved under various network conditions and topoligies is further discussed in @eval.

#pagebreak()

= Theoretical Analysis <theoretical-analysis>

This section provides a theoretical analysis of the proposed solution, evaluating the conditions under which it can achieve desired consistency properties of a realtime collaboration system. As a first step, a generalisation of the proposed solution (as detailed in @system-design) is provided (@general-form), followed by a detailed evaluation of this general form under various network conditions, alongside evaluation of applicability of this general criteria to the specific case of the proposed solution (@eval). In the later parts of this chapter, results and limitations of these theoretical results are discussed.
...
- We ensure strong indempotency properties for operations within each scope, allowing for a mechanism to determine the _winning order_ or the _winning operation_ within each scope, with complexity of said mechanism solely determined by the feasible network topology (e.g. a simple relay with a client and a server).

Based on these, various different actual mechanisms can be designed and implemented, ranging from a naive relay to a fully distributed setup, either treating the operations as blackboxes, or by coupling to the "scope" they affect.

This notion can be generalised using the introduced concept of _lenses_, which are arbitrary functions mapping data to some other data type. Operations can be then categorised based on lenses which they affect (i.e. the change they make is reflected by the lens), establishing _operational scopes_. Commutativity between operations of different scopes can then be achieved if the original data-type can be reconstructed from lenses associated with these scopes.

== General Form <general-form>

=== Definitions <defs>

Firstly, we  assume a data type (an arbitrary set) $D$, equiped with an equality operator $=$, and a set of operations $O_D$ in the general form of $f: D -> D$. Within this context, we define a _lens_ as an arbitrary function from $D$ to some other domain $D'$ similarly equiped with an equality operator, i.e. a _lens_ is a function $ell: D -> D'$. For a given set of lenses, $Gamma_D$, the target domain $D'$ is determined implicitly as a set of all possible domains of members of $Gamma_D$, constructed in any arbitrary manner from its constintuents, but specifically in a manner that still provides an equality operator. For readability, we refer to the result of applying lens $ell$ on data $d$ as _$d$ viewed through (or under) $ell$_ and denote it by $d_ell$.
...
#definition[
  Two operations _share scope_ iff any lens that sees one, also sees the other. We denote this relationship by the $sharesscope$ symbol, and for all $f, g in O_D$:
...
Subsequently, we provide definitions to capture the notion of _independence_ of operations of varying scopes:

#definition[
  Operation $f$ _is blind to operation_ $g$ under lens $ell$, iff:
...
#definition[
  A given lens $ell$ _is blind to operation_ $f$, iff it does not see the effect of $f$ on any data. We denote this relationship by the $blindto$ symbol, and have:
...
#definition[
  ...
  Where application and composition of ordered sets of operations is equivalent to application and composition of their elements in order, i.e.
...
#proposition[
  ...
  #proof[
    ...
    For any larger sequence $X$, we break it into smaller sequences $X_1$ and $X_2$. Using induction, we have:
...
It is notable that the operations of the proposed the realtime collaboration solution display a stronger property than _strong idempotency_: they effectively _overwrite_ the content of their respective scopes. This notion can be expressed by an operation being blind to other operations of a scope, including potentially itself, under the selectors of the scope.
...
In such a scenario, it suffices for the server to broadcast the operations it receives to all clients, including the client who issued the operation. Assuming client A conducts operation $a$ while client B conducts operation $b$ locally, and the server broadcasts the sequence $angle.l a, b angle.r$. Assuming both clients started from the same initial state $i$, its easy to observe how their respective states, denoted by $S_A$ and $S_B$ converge:
...
Provided analysis merely hints at the connection between the outlined general framework and CRDTs, leaving room for more in-depth analysis of properties of CRDTs that can be relaxed in favor of simplicity and performance, and the implications of such relaxations on consistency. The perceived "simplicity" that is achieved here is also only described qualitatively, and while in comparison to CRDTs where information about pariticpants is embedded into the data type itself, or in cases where deriving the representational data from the replicated schema is non-trivial, this might be easy to express more precisely, present work falls short of providing such an expression, or further investigation with cases where distinctions are not trivial.

#pagebreak()

= Summary <summary>

In the following sections, we summarize the status of the thesis, providing an overview of fulfillment of outlined functional and non-functional requirements, and the achieved and open goals. We conclude by providing an overview of the contribution made by this work and potential future work.
